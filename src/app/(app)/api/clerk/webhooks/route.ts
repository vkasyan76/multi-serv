...
export async function POST(req: Request) {
  ...
  if (eventType === "user.created") {
    ...
    if (findUser.docs.length === 0) {
      ...
      console.log("Webhook - Username resolved:", {
        original: clerkUsername,
        resolved: unique,
      });
  ...
  if (eventType === "user.updated") {
    ...
    if (findUser.docs.length > 0) {
      ...
      if (existingUser) {
        const primaryEmailId = evt.data.primary_email_address_id;
        const email =
          email_addresses?.find((e) => e.id === primaryEmailId)
            ?.email_address ??
          email_addresses?.[0]?.email_address ??
          null;
        ...
        if (existingUser) {
          ...
          if (existingUser.tenants && existingUser.tenants.length > 0) {
            ...
            for (const tenantRef of existingUser.tenants) {
              const tenantId =
                typeof tenantRef.tenant === "object"
                  ? tenantRef.tenant.id
                  : tenantRef.tenant;
              ...
              try {
                ...
                // Robustly resolve image id (works for string or populated object)
                const imageId =
                  typeof tenant?.image === "string"
                    ? tenant.image
                    : tenant?.image &&
                        typeof tenant.image === "object" &&
                        "id" in tenant.image
                      ? (tenant.image as { id: string }).id
                      : undefined;
                ...
                if (imageId) {
                  try {
                    ...
                    console.log("Deleted tenant image:", imageId);
                  } catch (mediaError) {
                    console.error(
                ...
                // NEW: Best-effort Stripe connected account cleanup user/tenant in your app and trying to delete the connected account does not erase payment records from the Stripe dashboard
                const acctId =
                ...
                if (acctId) {
                  try {
                    ...
                    console.log("Deleted Stripe connected account:", acctId);
                  } catch (err) {
                    // This can fail if the account ever moved money (esp. in live mode).
                    // That's OK—log and continue removing app data.
                    console.warn(
