...
export default function TenantContent({ slug }: { slug: string }) {
  ...
  const handleContact = async () => {
    // If bridge isn't ready yet, open sheet (it will show "Checking sign-in…")
    if (!bridge?.ok) {
      setChatOpen(true);
      return;
    }

    // If bridge says "not authed", re-check once immediately before toasting.
    if (bridge.authenticated === false) {
      const fresh = await refetchBridge(); // see note below
      if (fresh.data?.ok && fresh.data.authenticated === false) {
        toast.error("Sign in to contact this provider.");
        return;
      }
    }
