...
type MessageReceivedTenantTemplateProps = {
...
function MessageReceivedTenantEmail(props: MessageReceivedTenantTemplateProps) {
  const greeting = props.recipientName?.trim()
    ? `Dear ${props.recipientName.trim()},`
    : "Dear,";
  const from = props.senderName?.trim() || "a customer";
...
export async function renderMessageReceivedTenantTemplate(
  ...
  if (!ctaUrl) {
    throw new Error(
      "ctaUrl is required for message-received-tenant template",
    );
  }
  ...
  const text = [
    ...
    "",
    `You received a new message from ${safeSenderName || "a customer"}.`,
    "",
    messagePreview ? `Preview: ${messagePreview}` : undefined,
    "",
    `Open Messages: ${ctaUrl}`,
  ]
